...
class SpeedController:
    ...
    def update(self, theta1, theta2, theta3, theta4, dt, t):     
        ...
        if t<time_shift:
            ...
            error13 = -(theta3 - theta1 - 2*2*(np.pi/4))
            error14 = -(theta4 - theta1 - 3*2*(np.pi/4))
            error_net_new = error12 + error13 + error14 
...
class SpeedController2:
    ...
    def update(self, theta1, theta2, theta3, theta4, dt, t):     
        ...
        if t<time_shift:
            ...
            error24 = -(theta4 - theta2 - 2*2*(np.pi/4))
            error_net2 = error21 + error23 + error24
...
class SpeedController3:
    ...
    def update(self, theta1, theta2, theta3, theta4, dt, t):
        ...
        if t<time_shift: 
            ...
            #Formation Error
            error31 =  (theta3 - theta1 - 2*2*(np.pi/4))
            error32 =  (theta3 - theta2 - 1*2*(np.pi/4))
            error34 = -(theta4 - theta3 - 1*2*(np.pi/4))
            error_net_3 = error31 + error32 + error34 
...
class SpeedController4:
    ...
    def update(self, theta1, theta2, theta3, theta4, dt, t):
        ...
        if t<time_shift:
            ...
            output4p = self.kp * error4 + error_der4p
            #Formation Error
            error41 =  (theta4 - theta1 - 3*2*(np.pi/4))
            error42 =  (theta4 - theta2 - 2*2*(np.pi/4))
            error43 =  (theta4 - theta3 - 1*2*(np.pi/4))
            error_net_4 = error41 + error42 + error43 
