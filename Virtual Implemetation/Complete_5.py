...
class SpeedController:
    ...
    def update(self, theta1, theta2, theta3, theta4, theta5, dt, t):       
        ...
        if t<time_shift:
            ...
            error12 = -(theta2 - theta1 - 1*2*(np.pi/5))
            error13 =  (theta3 - theta1 - 2*2*(np.pi/5))
            error14 = -(theta4 - theta1 - 3*2*(np.pi/5))
            error15 =  (theta5 - theta1 - 4*2*(np.pi/5))
            error_net_new = error12 + error13 + error14 + error15
...
class SpeedController2:
    ...
    def update(self, theta1, theta2, theta3, theta4, theta5, dt, t):
        ...
        if t<time_shift:
            ...
            error21 =  (theta2 - theta1 - 1*2*(np.pi/5))
            error23 = -(theta3 - theta2 - 1*2*(np.pi/5))
            error24 =  (theta4 - theta2 - 2*2*(np.pi/5))
            error25 = -(theta5 - theta2 - 3*2*(np.pi/5))

            error_net2 = error21 + error23 + error24 + error25
...
class SpeedController3:
    ...
    def update(self, theta1, theta2, theta3, theta4, theta5, dt, t):
        if t<time_shift:   
            ...
            error31 = -(theta3 - theta1 - 2*2*(np.pi/5))
            error32 =  (theta3 - theta2 - 1*2*(np.pi/5))
            error34 = -(theta4 - theta3 - 1*2*(np.pi/5))
            error35 =  (theta5 - theta3 - 2*2*(np.pi/5))

            error_net_3 = error31 + error32 + error34 + error35
...
class SpeedController4:
    ...
    def update(self, theta1, theta2, theta3, theta4, theta5, dt, t):
        ...
        if t<time_shift:
            ...
            error41 =  (theta4 - theta1 - 3*2*(np.pi/5))
            error42 = -(theta4 - theta2 - 2*2*(np.pi/5))
            error43 =  (theta4 - theta3 - 1*2*(np.pi/5))
            error45 = -(theta5 - theta4 - 1*2*(np.pi/5))
            error_net_4 = error41 + error42 + error43 +  error45
...
class SpeedController5:
    ...
    def update(self, theta1, theta2, theta3, theta4, theta5, dt, t):
        if t<time_shift:          
            ...
            output5p = self.kp * error5 + error_der5s
            #Formation Error
            error51 = -(theta5 - theta1 - 4*2*(np.pi/5))
            error52 =  (theta5 - theta2 - 3*2*(np.pi/5))
            error53 = -(theta5 - theta3 - 2*2*(np.pi/5))
            error54 =  (theta5 - theta4 - 1*2*(np.pi/5))
            error_net_5 = error51 + error52 + error53 + error54
